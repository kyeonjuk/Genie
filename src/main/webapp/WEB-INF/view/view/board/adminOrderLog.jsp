 ...
 --%>
						...
						<table width="760" border="0" cellpadding="0" cellspacing="0"
							...
							<tbody>
								<c:if test="${not empty li}">
									<c:forEach var="b" items="${li}">
										<tr>
											<td>${logcount }</td>
											<td>${b.regdate}</td>
											<td>ORDERID : <a href="${pageContext.request.contextPath}/admin/adminOrderLogDetail?orderid=${b.orderid }" style="text-decoration: underline;">${b.orderid }</td>
											<td>${b.total }</td>
											<td>${b.orderstate}</td>
											<td>
											<c:if test="${b.orderstate=='주문접수' }">
												<button type="button" class="cart__list__orderbtn" onclick="location.href='${pageContext.request.contextPath}/admin/adminOrderState?orderid=${b.orderid}&state=배송중'">배송처리</button>
												<button type="button" class="cart__list__orderbtn" onclick="location.href='${pageContext.request.contextPath}/admin/adminOrderState?orderid=${b.orderid}&state=주문실패'">주문거절</button>
											</c:if>
											
											<c:if test="${b.orderstate=='취소신청' }"><button type="button" class="cart__list__orderbtn" onclick="location.href='${pageContext.request.contextPath}/admin/adminOrderState?orderid=${b.orderid}&state=주문취소'">취소확인</button></c:if>
											<c:if test="${b.orderstate=='배송중' }"><button type="button" class="cart__list__orderbtn" onclick="location.href='${pageContext.request.contextPath}/admin/adminOrderState?orderid=${b.orderid}&state=배송완료'">배송완료</button></c:if>
											<c:if test="${b.orderstate=='배송완료' }"><button type="button" class="cart__list__orderbtn" disalbed>수취대기</button></c:if>
											<c:if test="${b.orderstate=='수취완료' }"><button type="button" class="cart__list__orderbtn" disalbed>거래종료</button></c:if>
											</td>
										</tr>
								...
								<c:if test="${empty li}">
									<tr>
										<td colspan="4" class="alt" height="100">최근 주문 내역이 없습니다.</td>
									</tr>
								</c:if>
							</tbody>
						</table>
